...
export default function About() {
  ...
  useLayoutEffect(() => {
    ...
    // Small delay to ensure any global ScrollTrigger changes from other components have settled
    const timer = setTimeout(() => {
      ...
      // Disable pinning and collage animations on mobile
      ...
      const tl = gsap.timeline({
        ...
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            const collage = q('.hero-collage')[0]
            if (collage) {
              gsap.set(collage, { zIndex: above ? 5 : 1 })
            }
          },
        }
      })
  ...
  // Name style cycling effect
  useEffect(() => {
    const nameElement = nameRef.current
    if (!nameElement) return

    const styles = ['name-style-1', 'name-style-2', 'name-style-3', 'name-style-4', 'name-style-5', 'name-style-6', 'name-style-7']
    let currentIndex = 0

    const cycleStyle = () => {
      // Remove all style classes
      styles.forEach(style => nameElement.classList.remove(style))
      // Add current style
      nameElement.classList.add(styles[currentIndex])
      // Move to next style
    ...
    <section ref={rootRef} className="hero" id="about" aria-labelledby="hero-title">
      <div className="container">
        <h1 id="hero-title" className="hero__title text-blue">
          Hello, I'm <mark ref={nameRef} className="hl">Mehul</mark>
          <br />
          Welcome to my zone!
        </h1>
        <div className="cta-row">
          <a className="cta-big" href="https://www.linkedin.com/in/mehul-shetty/" target="_blank" rel="noopener noreferrer">Follow Me</a>
        </div>
      </div>
